...
from IMProToo_tools import unix2date, date2unix, limitMaInidces, quantile, oneD2twoD, _get_netCDF_module
...
class MrrZe:
  ...
  def __init__(self,rawData):
    ...
    #mrr frequency, MRR after 2011 (or upgraded) use 24.23e9    24.15e9
    self.co["mrrFrequency"] = 24.23e9 #in Hz,
    #wavelength in m
    ...
    self.co["maxH"] = rawData.mrrRawSpectrum.shape[1] - 1 # 31 # start couning at 0
    ...
    spectrum_border_min_list = np.zeros(rawData.mrrRawSpectrum.shape[1] - self.co["minH"], dtype=int) + 2
    spectrum_border_min_list[:3] = [5, 4, 3]
    spectrum_border_min_list[-3:] = [3, 3, 5]

    spectrum_border_max_list = np.zeros(rawData.mrrRawSpectrum.shape[1] - self.co["minH"], dtype=int) + 63
    spectrum_border_max_list[:3] = [60, 61, 62]
    spectrum_border_max_list[-3:] = [62, 61, 63]

    self.co["spectrumBorderMin"] = spectrum_border_min_list
    self.co["spectrumBorderMax"] = spectrum_border_max_list
    ...
    self.co["completelyMaskedHeights"] = [0, 1, rawData.mrrRawSpectrum.shape[1] - 2]
  ...
  def _cleanUpNoiseMask(self,spectrum):
    ...
    maxs[2:-2,2:-2] = np.ma.masked_array(np.ma.argmax(spectrum,axis=-1),noiseMask)[:,2:spectrum.shape[1]-1]
    ...
    for t in np.arange(self.no_t):
      ...
      for h in np.arange(2,self.co["maxH"]-1):
        if  noiseMask[t,h] == False:
          ...
          if t in [0,self.no_t-1] or h in [2,self.co["maxH"]-2]:
            limit = lowestLimit
          elif t in [1,self.no_t-2] or h in [3,self.co["maxH"]-3]:
            limit = lowLimit 
          else:
            limit = highLimit
  ...
  def _findHeightsForPeaks(self,extendedRawSpectrum,trustedPeakNo,trustedPeakVel,trustedPeakHeight,trustedPeakHeightStart,trustedPeakHeightStop,allPeaks, allPeaksIndices, allPeaksVelMe, allPeaksHeight):
    ...
    for t in np.arange(self.no_t):
      if t in self._allPeaks.keys():
        ...
        concatenated_range = chain(range(trustedPeakNo[t]-1,-1,-1), range(trustedPeakNo[t]+1,len(allPeaks[t])))
        for jj in concatenated_range:
...
class mrrProcessedData:
  ...
  def __init__(self,fname,debugLimit = 0,maskData=True,verbosity=2,ncForm="NETCDF3_CLASSIC"):
    ...
    for f,file in enumerate(files):
      ...
      aveTimestamps=          np.array(np.sort(list(dataMRR.keys()))[0:debugLimit],dtype=int)
